...
/**
 * 当这个插件开启时，hydrogen-store的state会同步到redux-dev-tool
 * 具体来说，开启这个插件的时候，所有的store操作被redux接管，并在redux数据变化后通知到hydrogen-store更新数据
 * @param store hydrogen-store的store实例
...
export default function openRedux(store, reduxStoreName, initialState, reducers) {
  ...
  // 当redux store的state变化时，触发hydrogen-store的数据变更
