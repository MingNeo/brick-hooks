...
```javascript
import { useStore } from 'air-store'
...
```javascript
import { registerModule, useStore } from 'air-store'
...
#### 使用redux devtool
即使因为种种原因你不想使用redux了，但是否时常会怀念redux devtool的方便？air-store 可以方便开启redux-devtool。仅需引入一个插件

```javascript
import { usePlugin, store } from 'air-store'
...
```javascript
import { createStore, Store } from 'air-store'
import reduxPlugin from 'air-store-redux-plugin'
...
虽然使用air-store可以很方便的存储全局状态，但是并不应该把所有数据都放在store中！
